Public Class frmHRIS_PerformanceManagement_AddUpdatePart1Form2_8_Projectsp1

    Private Sub frmHRIS_PerformanceManagement_AddUpdatePart1Form2_8_Projectsp1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ClearMatchingTextBoxes(Me)
        txtEmployeeName.Text = _EmployeeName
        txtDepartment.Text = _EmployeeDepartment
        txtPosition.Text = _EmployeePosition
        txtCoveredPeriod.Text = _StartDate.ToString("MMM/dd/yyyy") & " - " & _EndDate.ToString("MMM/dd/yyyy")
        btxtProjectName1.Text = atxtProjectName1.Text
        btxtProjectName2.Text = atxtProjectName2.Text
        btxtProjectName3.Text = atxtProjectName3.Text
        btxtProjectName4.Text = atxtProjectName4.Text
        btxtProjectName5.Text = atxtProjectName5.Text
        btxtProjectName6.Text = atxtProjectName6.Text
        btxtProjectName7.Text = atxtProjectName7.Text
        btxtProjectName8.Text = atxtProjectName8.Text
    End Sub

    Private Sub ClearMatchingTextBoxes(parent As Control) 'to clear the textboxes that has default values
        For Each ctrl As Control In parent.Controls
            If TypeOf ctrl Is TextBox AndAlso ctrl.Text = "text" OrElse ctrl.Text = "0.00" Then
                ctrl.Text = ""
            End If
            If ctrl.HasChildren Then
                ClearMatchingTextBoxes(ctrl)
            End If
        Next
    End Sub

    Private Sub btnNextTab1_Click(sender As Object, e As EventArgs) Handles btnNextTab1.Click
        TabControlRatings.SelectedTab = p2
    End Sub

    Private Sub btnNextTab2_Click(sender As Object, e As EventArgs) Handles btnNextTab2.Click
        TabControlRatings.SelectedTab = p3
    End Sub

    Private Sub btnNextTab3_Click(sender As Object, e As EventArgs) Handles btnNextTab3.Click
        TabControlRatings.SelectedTab = p4
    End Sub

    Private Sub btnNextTab4_Click(sender As Object, e As EventArgs) Handles btnNextTab4.Click
        TabControlRatings.SelectedTab = p5
    End Sub

    Private Sub btnNextTab5_Click(sender As Object, e As EventArgs) Handles btnNextTab5.Click
        TabControlRatings.SelectedTab = p6
    End Sub

    Private Sub btnNextTab6_Click(sender As Object, e As EventArgs) Handles btnNextTab6.Click
        TabControlRatings.SelectedTab = p7
    End Sub

    Private Sub btnNextTab7_Click(sender As Object, e As EventArgs) Handles btnNextTab7.Click
        TabControlRatings.SelectedTab = p8
    End Sub

    Private Sub btnPrevTab8_Click(sender As Object, e As EventArgs) Handles btnPrevTab8.Click
        TabControlRatings.SelectedTab = p7
    End Sub

    Private Sub btnPrevTab7_Click(sender As Object, e As EventArgs) Handles btnPrevTab7.Click
        TabControlRatings.SelectedTab = p6
    End Sub

    Private Sub btnPrevTab6_Click(sender As Object, e As EventArgs) Handles btnPrevTab6.Click
        TabControlRatings.SelectedTab = p5
    End Sub

    Private Sub btnPrevTab5_Click(sender As Object, e As EventArgs) Handles btnPrevTab5.Click
        TabControlRatings.SelectedTab = p4
    End Sub

    Private Sub btnPrevTab4_Click(sender As Object, e As EventArgs) Handles btnPrevTab4.Click
        TabControlRatings.SelectedTab = p3
    End Sub

    Private Sub btnPrevTab3_Click(sender As Object, e As EventArgs) Handles btnPrevTab3.Click
        TabControlRatings.SelectedTab = p2
    End Sub

    Private Sub btnPrevTab2_Click(sender As Object, e As EventArgs) Handles btnPrevTab2.Click
        TabControlRatings.SelectedTab = p1
    End Sub

    Private Sub btnBackTab1_Click(sender As Object, e As EventArgs) Handles btnBackTab1.Click
        Me.Close()
    End Sub

    Private Sub SetRatingAcronym(inputBox As TextBox, outputBox As TextBox)
        Dim r As Double
        If Double.TryParse(inputBox.Text, r) Then
            outputBox.Text = If(r >= 4.51, "E",
                        If(r >= 3.76, "EE",
                        If(r >= 3.0, "ME",
                        If(r >= 2.21, "MSE", "DE"))))
        Else
            outputBox.Text = ""
        End If
    End Sub

    Private Sub txtP1Rating1_TextChanged(sender As Object, e As EventArgs) Handles txtP1Rating1.TextChanged
        SetRatingAcronym(txtP1Rating1, txtP1R1Acro)
    End Sub

    Private Sub txtP1Rating2_TextChanged(sender As Object, e As EventArgs) Handles txtP1Rating2.TextChanged
        SetRatingAcronym(txtP1Rating2, txtP1R2Acro)
    End Sub

    Private Sub txtP1Rating3_TextChanged(sender As Object, e As EventArgs) Handles txtP1Rating3.TextChanged
        SetRatingAcronym(txtP1Rating3, txtP1R3Acro)
    End Sub

    Private Sub txtP1FW3_TextChanged(sender As Object, e As EventArgs) Handles txtP1FW3.TextChanged
        txtP1FWT.Text = (Val(txtP1FW3.Text) + Val(txtP1FW2.Text) + Val(txtP1FW1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP1WS3_TextChanged(sender As Object, e As EventArgs) Handles txtP1WS3.TextChanged
        txtP1WST.Text = (Val(txtP1WS3.Text) + Val(txtP1WS2.Text) + Val(txtP1WS1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP2Rating1_TextChanged(sender As Object, e As EventArgs) Handles txtP2Rating1.TextChanged
        SetRatingAcronym(txtP2Rating1, txtP2R1Acro)
    End Sub

    Private Sub txtP2Rating2_TextChanged(sender As Object, e As EventArgs) Handles txtP2Rating2.TextChanged
        SetRatingAcronym(txtP2Rating2, txtP2R2Acro)
    End Sub

    Private Sub txtP2Rating3_TextChanged(sender As Object, e As EventArgs) Handles txtP2Rating3.TextChanged
        SetRatingAcronym(txtP2Rating3, txtP2R3Acro)
    End Sub

    Private Sub txtP2FW3_TextChanged(sender As Object, e As EventArgs) Handles txtP2FW3.TextChanged
        txtP2FWT.Text = (Val(txtP2FW3.Text) + Val(txtP2FW2.Text) + Val(txtP2FW1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP2WS3_TextChanged(sender As Object, e As EventArgs) Handles txtP2WS3.TextChanged
        txtP2WST.Text = (Val(txtP2WS3.Text) + Val(txtP2WS2.Text) + Val(txtP2WS1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP3Rating1_TextChanged(sender As Object, e As EventArgs) Handles txtP3Rating1.TextChanged
        SetRatingAcronym(txtP3Rating1, txtP3R1Acro)
    End Sub

    Private Sub txtP3Rating2_TextChanged(sender As Object, e As EventArgs) Handles txtP3Rating2.TextChanged
        SetRatingAcronym(txtP3Rating2, txtP3R2Acro)
    End Sub

    Private Sub txtP3Rating3_TextChanged(sender As Object, e As EventArgs) Handles txtP3Rating3.TextChanged
        SetRatingAcronym(txtP3Rating3, txtP3R3Acro)
    End Sub

    Private Sub txtP3FW3_TextChanged(sender As Object, e As EventArgs) Handles txtP3FW3.TextChanged
        txtP3FWT.Text = (Val(txtP3FW3.Text) + Val(txtP3FW2.Text) + Val(txtP3FW1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP3WS3_TextChanged(sender As Object, e As EventArgs) Handles txtP3WS3.TextChanged
        txtP3WST.Text = (Val(txtP3WS3.Text) + Val(txtP3WS2.Text) + Val(txtP3WS1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP4Rating1_TextChanged(sender As Object, e As EventArgs) Handles txtP4Rating1.TextChanged
        SetRatingAcronym(txtP4Rating1, txtP4R1Acro)
    End Sub

    Private Sub txtP4Rating2_TextChanged(sender As Object, e As EventArgs) Handles txtP4Rating2.TextChanged
        SetRatingAcronym(txtP4Rating2, txtP4R2Acro)
    End Sub

    Private Sub txtP4Rating3_TextChanged(sender As Object, e As EventArgs) Handles txtP4Rating3.TextChanged
        SetRatingAcronym(txtP4Rating3, txtP4R3Acro)
    End Sub

    Private Sub txtP4FW3_TextChanged(sender As Object, e As EventArgs) Handles txtP4FW3.TextChanged
        txtP4FWT.Text = (Val(txtP4FW3.Text) + Val(txtP4FW2.Text) + Val(txtP4FW1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP4WS3_TextChanged(sender As Object, e As EventArgs) Handles txtP4WS3.TextChanged
        txtP4WST.Text = (Val(txtP4WS3.Text) + Val(txtP4WS2.Text) + Val(txtP4WS1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP5Rating1_TextChanged(sender As Object, e As EventArgs) Handles txtP5Rating1.TextChanged
        SetRatingAcronym(txtP5Rating1, txtP5R1Acro)
    End Sub

    Private Sub txtP5Rating2_TextChanged(sender As Object, e As EventArgs) Handles txtP5Rating2.TextChanged
        SetRatingAcronym(txtP5Rating2, txtP5R2Acro)
    End Sub

    Private Sub txtP5Rating3_TextChanged(sender As Object, e As EventArgs) Handles txtP5Rating3.TextChanged
        SetRatingAcronym(txtP5Rating2, txtP5R3Acro)
    End Sub

    Private Sub txtP5FW3_TextChanged(sender As Object, e As EventArgs) Handles txtP5FW3.TextChanged
        txtP5FWT.Text = (Val(txtP5FW3.Text) + Val(txtP5FW2.Text) + Val(txtP5FW1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP5WS3_TextChanged(sender As Object, e As EventArgs) Handles txtP5WS3.TextChanged
        txtP5WST.Text = (Val(txtP5WS3.Text) + Val(txtP5WS2.Text) + Val(txtP5WS1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP6Rating1_TextChanged(sender As Object, e As EventArgs) Handles txtP6Rating1.TextChanged
        SetRatingAcronym(txtP6Rating1, txtP6R1Acro)
    End Sub

    Private Sub txtP6Rating2_TextChanged(sender As Object, e As EventArgs) Handles txtP6Rating2.TextChanged
        SetRatingAcronym(txtP6Rating2, txtP6R2Acro)
    End Sub

    Private Sub txtP6Rating3_TextChanged(sender As Object, e As EventArgs) Handles txtP6Rating3.TextChanged
        SetRatingAcronym(txtP6Rating3, txtP6R3Acro)
    End Sub

    Private Sub txtP6FW3_TextChanged(sender As Object, e As EventArgs) Handles txtP6FW3.TextChanged
        txtP6FWT.Text = (Val(txtP6FW3.Text) + Val(txtP6FW2.Text) + Val(txtP6FW1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP6WS3_TextChanged(sender As Object, e As EventArgs) Handles txtP6WS3.TextChanged
        txtP6WST.Text = (Val(txtP6WS3.Text) + Val(txtP6WS2.Text) + Val(txtP6WS1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP7Rating1_TextChanged(sender As Object, e As EventArgs) Handles txtP7Rating1.TextChanged
        SetRatingAcronym(txtP7Rating1, txtP7R1Acro)
    End Sub

    Private Sub txtP7Rating2_TextChanged(sender As Object, e As EventArgs) Handles txtP7Rating2.TextChanged
        SetRatingAcronym(txtP7Rating2, txtP7R2Acro)
    End Sub

    Private Sub txtP7Rating3_TextChanged(sender As Object, e As EventArgs) Handles txtP7Rating3.TextChanged
        SetRatingAcronym(txtP7Rating3, txtP7R3Acro)
    End Sub

    Private Sub txtP7FW3_TextChanged(sender As Object, e As EventArgs) Handles txtP7FW3.TextChanged
        txtP7FWT.Text = (Val(txtP7FW3.Text) + Val(txtP7FW2.Text) + Val(txtP7FW1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP7WS3_TextChanged(sender As Object, e As EventArgs) Handles txtP7WS3.TextChanged
        txtP7WST.Text = (Val(txtP7WS3.Text) + Val(txtP7WS2.Text) + Val(txtP7WS1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP8Rating1_TextChanged(sender As Object, e As EventArgs) Handles txtP8Rating1.TextChanged
        SetRatingAcronym(txtP8Rating1, txtP8R1Acro)
    End Sub

    Private Sub txtP8Rating2_TextChanged(sender As Object, e As EventArgs) Handles txtP8Rating2.TextChanged
        SetRatingAcronym(txtP8Rating2, txtP8R2Acro)
    End Sub

    Private Sub txtP8Rating3_TextChanged(sender As Object, e As EventArgs) Handles txtP8Rating3.TextChanged
        SetRatingAcronym(txtP8Rating3, txtP8R3Acro)
    End Sub

    Private Sub txtP8FW3_TextChanged(sender As Object, e As EventArgs) Handles txtP8FW3.TextChanged
        txtP8FWT.Text = (Val(txtP8FW3.Text) + Val(txtP8FW2.Text) + Val(txtP8FW1.Text)).ToString("0.00")
    End Sub

    Private Sub txtP8WS3_TextChanged(sender As Object, e As EventArgs) Handles txtP8WS3.TextChanged
        txtP8WST.Text = (Val(txtP8WS3.Text) + Val(txtP8WS2.Text) + Val(txtP8WS1.Text)).ToString("0.00")
    End Sub

    Private Sub btnNextTab8_Click(sender As Object, e As EventArgs) Handles btnNextTab8.Click
        frmHRIS_PerformanceManagement_AddUpdatePart1Form2_4_Projectsp2.ShowDialog()
    End Sub

End Class